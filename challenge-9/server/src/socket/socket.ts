...
io.on('connection',async (socket)=>{
    ...
    socket.on("message", async (message) => {
      ...
      // socket.emit('recieved-message',msg + ' from server')
        ...
        !exist ? await RoomModel.create({roomName:message.room,conversation:[{...message_info}]}) : await RoomModel.updateMany({roomName: message.room},{ $push: { conversation: {...message_info} } })
        message.room === '' ? socket.broadcast.emit("recieved-message", message_info) : socket.to(message.room).emit("recieved-message", message_info);
    });
